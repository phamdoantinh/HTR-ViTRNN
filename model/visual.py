...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward(self, x, mask_ratio=0.0, max_span_length=1, use_masking=False, return_debug=False):
        x = self.layer_norm(x)
        ...
        if return_debug:
            debug_dict = {
                "x_before_rnn": x_before_rnn.detach(),
                "x_after_rnn": x_after_rnn.detach(),
                "logit_before_raw": logit_before_raw.detach(),
                "logit_after_raw": logit_after_raw.detach(),
            }
            return logits, debug_dict
